# ALFRED: first run on Windows

## Download and launch

1. Use Windows 10 or 11, 64-bit (x64). These builds are for Windows desktop PCs.
2. Download **ALFRED.exe** from this repository's Releases page. The GitHub source-code ZIP is for development; it is not the app download.
...
```powershell
Get-FileHash .\ALFRED.exe -Algorithm SHA256
```
...
Try typing **what time is it** or **open Notepad** first. Local PC commands work without an AI key. Then press **Talk** and say a request. Enable **Wake word** to call “Alfred” before a question. For reliable capture, run one assistant with wake listening enabled at a time. Wake listening pauses during replies to avoid feedback from the speaker.
...
Settings, keys and personal history are stored in `%LOCALAPPDATA%\FanAssistants\ALFRED\`, separately from the EXE. Moving or updating the EXE keeps those settings.
