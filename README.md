...
- Keep ALFRED in the tray and optionally start it when you sign in to Windows.
...
4. Enable **Wake word**, then say “Alfred, open Spotify” or press **Talk**.
...
The tray menu has **Show**, speech mute, wake toggle and **Exit**. **Ctrl+Q** exits from the main window. A small gold indicator pulses when Alfred is detected, changes during processing/speech, and fades after the exchange. Temporary microphone failures retry automatically.
...
```text
%LOCALAPPDATA%\FanAssistants\ALFRED\
```
...
```text
%LOCALAPPDATA%\FanAssistants\ALFRED\build-output\dist\ALFRED.exe
```
